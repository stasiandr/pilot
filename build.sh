...
if [[ -f .build/jadx/engine.jar ]]; then
    mkdir -p "$APP/Contents/Resources/Jadx"
    cp .build/jadx/engine.jar .build/jadx/jadx.jar "$APP/Contents/Resources/Jadx/"
    # В jadx-all лежат нативные библиотеки: flatlaf для GUI jadx и j2v8 под
    # Linux. Pilot зовёт только ядро, а неподписанный .dylib внутри jar
...
if [[ -x .build/netcoredbg/netcoredbg ]]; then
    ...
    cp -R .build/netcoredbg/ "$APP/Contents/Resources/netcoredbg/"
    rm -f "$APP/Contents/Resources/netcoredbg/VERSION"
fi
...
# ad-hoc: macOS не даст приложению доступ к файлам и будет ругаться при
# каждом запуске, если не подписать хоть так.
if [[ -n "${SIGN_ID:-}" ]]; then
