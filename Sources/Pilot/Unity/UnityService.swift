...
final class UnityService: ObservableObject {
    ...
    func indexAssets() {
        ...
        queue.async { [weak self] in
            // Индекс прошлого запуска — сразу: ссылки на ассеты подписаны с
            // первой секунды. Он мог устареть, поэтому обход всё равно идёт,
            // а `isIndexingAssets` до его конца остаётся поднятым.
            let cached = IndexCache.loadAssets(root: project.root)
            if let cached {
                Task { @MainActor in
                    guard let self, counter.isCurrent(current), self.assets == nil else { return }
                    NSLog("[unity] индекс GUID из кэша: %d ассетов", cached.index.count)
                    self.assets = cached.index
                    self.decorationsVersion += 1
                    self.onAssetsReady?()
                }
            }
            // Обход всё равно полный — но читаются только `.meta`, менявшиеся
            // после прошлой сборки; остальные GUID — из кэша.
            let builtFrom = Date()
            let index = UnityAssetIndex.build(root: project.root, reusing: cached?.index,
    ...
    func definition(in document: LoadedDocument, at offset: Int) async -> Jump {
        ...
        case .local(let fileID):
            ...
            guard let file, let object = file.object(fileID) else {
                return .unavailable(L("Объекта &\(fileID) в этом файле нет"))
            }
    ...
    func target(forAsset guid: UnityGUID, fileID: Int64?) async -> Jump {
        if guid.isBuiltin { return .unavailable(L("Встроенный ресурс Unity — в проекте его нет")) }
        guard let assets else { return .unavailable(L("Индекс ассетов ещё строится…")) }
        guard let path = assets.path(for: guid), let url = url(forAsset: path) else {
            return .unavailable(L("Ассет \(guid) не найден — битая ссылка"))
        }
    ...
    enum UsageError: Error {
        ...
        var message: String {
            switch self {
            case .notUnity: return L("Это не Unity-проект")
            case .indexing: return L("Индекс ассетов ещё строится — попробуйте через секунду")
            case .noMeta:   return L("У файла нет .meta — Unity о нём не знает")
            }
        }
    }
    ...
    func decorator() -> CodeDecorator? {
        ...
        return { document, range in
            ...
            for (guidRange, guid) in UnityYAMLFile.guidRanges(in: units, visible) {
                if guid.isBuiltin {
                    result.append(TextDecoration(range: guidRange, toolTip: L("Встроенный ресурс Unity")))
                } else if let path = assets?.path(for: guid) {
                    result.append(TextDecoration(
                        range: guidRange, color: Theme.assetLink, underline: true,
                        toolTip: L("\(UnityProjectInfo.prettyPath(path))\n⌘B или ⌘+клик — открыть")))
                } else if complete {
                    result.append(TextDecoration(
                        range: guidRange, color: Theme.brokenLink,
                        toolTip: L("Ассета с таким GUID в проекте нет — битая ссылка (Missing)")))
                }
            }
            ...
            if let file = document.unityFile, document.isSemanticsFresh {
                ...
                for (refRange, fileID) in UnityYAMLFile.localReferences(in: units, visible) {
                    ...
                    let what = object.stripped
                        ? L("\(object.typeName) из вложенного префаба")
