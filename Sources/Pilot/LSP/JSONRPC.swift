...
enum RPCError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .malformed(let what):                return L("Некорректный ответ LSP: \(what)")
        case .timeout(let method):                return L("Таймаут запроса \(method)")
        case .notRunning:                         return L("Языковой сервер не запущен")
        case .cancelled:                          return L("Запрос отменён")
        }
    }
}
...
enum JSON {
    ...
    static func decode<T: Decodable>(_ type: T.Type, from value: Any) throws -> T {
        if value is NSNull { throw RPCError.malformed(L("получен null вместо \(T.self)")) }
