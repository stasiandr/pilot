...
/// Быстрый навигатор: ⌘B, ⌘T и ⌘R без компилятора.
///
/// Работает с первой секунды по индексу объявлений. Для C# первым отвечает
/// Rustlyn (см. `RustlynNavigator`): компилятор, когда проект скомпилирован,
/// и его индекс объявлений до этого; этот разбор — для остального и на
/// случай, когда тот не знает. Семантики нет, есть приближение к ней:
///
...
/// и сборок без исходников. Там отвечает Rustlyn, когда проект скомпилирован.
struct LocalNavigator {
    ...
    func resolveType(_ raw: String, context: SourceFileInfo, depth: Int = 0) -> ResolvedType? {
        ...
        guard !Self.builtinTypes.contains(base),
              var candidates = index.typesByName[base], !candidates.isEmpty else { return nil }
        // Тип, вложенный в чужой тип, по голому имени снаружи не виден:
        // `Vector3` в файле с `using UnityEngine` — движковый, а не
        // `struct Vector3` из теста пакета. Не нашлось видимого — не
        // ответ: дальше спросят сборки.
        if qualifier == nil {
            candidates = candidates.filter { !isHiddenNested($0, from: context) }
    ...
    /// Вложен в тип, объявленный в другом файле: по голому имени оттуда
    /// его не видно. Контейнер у вложенного типа — имя внешнего типа, у
    /// типа верхнего уровня — namespace его файла.
    private func isHiddenNested(_ id: Int32, from context: SourceFileInfo) -> Bool {
        guard let index, !SymbolIndex.isMember(index[id].kind),
              let container = index[id].container, !container.isEmpty else { return false }
        let info = index.fileInfo(id)
        if info.path == context.path || info.namespaces.contains(container) { return false }
        let outer = container.split(separator: ".").last.map(String.init) ?? container
        return index.typesByName[outer]?.isEmpty == false
    }
    ...
    private func fallback(_ name: String, preferMembers: Bool, at offset: Int, allowJump: Bool = true) -> Answer {
        ...
        // Один кандидат — ещё не ответ, если отсюда его не видно.
        let visible = ranked.first.map { !isHiddenNested($0.0, from: fileInfo) } ?? false
        return Answer(declarations: top, isExact: allowJump && top.count == 1 && visible)
    }
