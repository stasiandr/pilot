...
enum OutlineBuilder {
    ...
    /// Заголовки Markdown; вложенность — по уровню, а не по отступу.
    static func markdownHeadings(_ model: SyntaxModel) -> [OutlineItem] {
        guard model.lineCount <= maxLines else { return [] }
        let headings = Markdown.headings(model.text)
        let top = headings.map(\.level).min() ?? 1
        var parents: [(level: Int, title: String)] = []
        return headings.enumerated().map { index, heading in
            while let last = parents.last, last.level >= heading.level { parents.removeLast() }
            let container = parents.last?.title
            parents.append((heading.level, heading.title))
            let line = min(heading.line, model.lineCount - 1)
            let bounds = model.lineRange(line)
            return OutlineItem(id: index, name: heading.title, kind: .heading,
                               range: NSRange(location: bounds.lowerBound, length: 0),
                               line: line, depth: heading.level - top, container: container)
        }
    }
    ...
    static func build(model: SyntaxModel) -> [OutlineItem] {
        if model.spec?.name == Languages.markdown.name { return markdownHeadings(model) }
        ...
        if let settledFile = model.settledFile, let rustlyn = Rustlyn.session(for: settledFile),
