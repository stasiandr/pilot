...
enum GitLabError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .unauthorized:         return L("GitLab не принял токен — возможно, он отозван или истёк")
        case .forbidden(let what):  return L("Нет прав: \(what)")
        case .notFound:             return L("Не найдено — проект или MR недоступны с этим токеном")
        case .http(let code, let message): return L("GitLab ответил \(code): \(message)")
        case .network(let why):     return L("Нет связи с GitLab: \(why)")
        case .decoding(let why):    return L("Непонятный ответ GitLab: \(why)")
        }
    }
}
...
struct GitLabClient: Sendable {
    ...
    private func perform(_ method: String, _ path: String, query: [String: String] = [:],
        ...
        guard var components = URLComponents(string: base + path) else {
            throw GitLabError.network(L("неверный адрес \(host)"))
        }
        if !query.isEmpty {
            components.queryItems = query.sorted { $0.key < $1.key }.map { URLQueryItem(name: $0.key, value: $0.value) }
        }
        guard let url = components.url else { throw GitLabError.network(L("неверный адрес \(host)")) }
        ...
        guard let http = response as? HTTPURLResponse else { throw GitLabError.network(L("нет ответа")) }
