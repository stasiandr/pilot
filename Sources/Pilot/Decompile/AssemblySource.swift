...
///
/// Исходников у собранной библиотеки нет, но есть метаданные и IL. Rustlyn
/// декомпилирует их обратно в C# вместе с телами методов, и этот текст
/// читают как обычный C#-файл — с подсветкой, структурой (`⌘⇧O`) и
/// переходами. Без Rustlyn текст собирается из одних метаданных: типы,
/// члены, сигнатуры, атрибуты и константы, но без тел.
enum AssemblySource {
    ...
    /// или битый файл. Синхронный и не из главного потока: первая
    /// декомпиляция `UnityEngine.CoreModule` — треть секунды, `UnityEditor`
    /// — секунды.
    ///
    /// Сборку читает Rustlyn, если он есть: декомпилирует IL в C# с телами
    /// методов, а IL отдельного метода отдаёт по запросу — см.
    /// `methodBody(of:line:)`. И его ответ кэшируется по отпечатку
    ...
    static func text(of url: URL) throws -> String {
        if let rustlyn = Rustlyn.session(for: url), let text = rustlyn.assemblyText(url) {
            return text
        }
    ...
    ///
    /// C# над ним — то, что делает IL, записанное обратно; IL — то, что в
    /// сборке лежит на самом деле. Когда декомпилятор где-то ошибся, IL
    /// это покажет.
    static func methodBody(of url: URL, line: Int) -> String? {
        guard let rustlyn = Rustlyn.session(for: url),
...
private extension AssemblyPrinter {
    ...
    func writeHeader(types: Int) {
        ...
        line(0, L("// Типов верхнего уровня: \(types). Из метаданных видны объявления,"))
        line(0, L("// тела методов в C# не восстанавливаются."))
    }
