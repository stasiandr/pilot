...
struct AssemblyMetadata {
    ...
    enum Failure: Error, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notManaged:      return "Нативная библиотека: .NET-метаданных в ней нет"
            case .damaged(let why): return "Не удалось прочитать сборку: \(why)"
            }
        }
    }
    ...
    init(bytes: [UInt8]) throws {
        ...
        guard let tables = tablesStream else { throw Failure.damaged("нет потока таблиц") }
        ...
        guard start <= bytes.count else { throw Failure.damaged("таблицы не помещаются в файл") }
    }
