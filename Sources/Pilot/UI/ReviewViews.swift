...
struct ReviewNavigator: View {
    ...
    private var content: some View {
        ...
        case .failed(let why):
            VStack(spacing: 10) {
                ...
                Button(L("Повторить")) { review.refreshList() }
            }
    ...
    private var connectPrompt: some View {
        VStack(spacing: 12) {
            ...
            Button(L("Подключить GitLab…")) { review.isConnectSheetPresented = true }
        }
        .padding()
    ...
    var body: some View {
        ...
        List {
            ...
            if searching {
                ...
                Text(L("Открытых мерж-реквестов нет"))
                    .font(.system(size: 12))
                    .foregroundStyle(.tertiary)
            }
            if let error = review.actionError {
    ...
    private var footer: some View {
        HStack(spacing: 8) {
            ...
            Button { review.refreshList() } label: { Image(systemName: "arrow.clockwise") }
                .help(L("Обновить список"))
            Menu {
                Button(L("Сменить токен…")) { review.isConnectSheetPresented = true }
                Button(L("Отключить GitLab"), role: .destructive) { review.disconnect() }
...
struct StateBadge: View {
    ...
    private var title: String {
        switch state {
        case "merged": return L("Слит")
        case "closed": return L("Закрыт")
        case "locked": return L("Заблокирован")
    ...
    var body: some View {
        if let active = review.active {
            List(selection: selection) {
                ...
                Section(L("Файлы · просмотрено \(active.reviewedCount) из \(active.files.count)")) {
                ...
                Section(L("Обсуждение")) {
                    ForEach(active.generalThreads) { thread in
                        ThreadCard(discussion: thread, review: review)
                            .listRowSeparator(.hidden)
                    }
                    CommentComposer(placeholder: L("Комментарий к мерж-реквесту…")) { body in
    ...
    private func header(_ active: ActiveReview) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 6) {
            Button { workspace.closeReview() } label: {
                Label(L("Все мерж-реквесты"), systemImage: "chevron.left")
            }
            .buttonStyle(.plain)
            .font(.system(size: 11))
            .foregroundStyle(.secondary)
            ...
            if let approvers = active.approvals?.approvedBy, !approvers.isEmpty {
                Text(L("Апрувы: \(approvers.map(\.user.name).joined(separator: ", "))"))
                    .font(.system(size: 11)).foregroundStyle(.secondary)
            }
            if let error = review.actionError {
                Text(error).font(.system(size: 11)).foregroundStyle(.orange)
...
struct ApproveButton: View {
    ...
    var body: some View {
        ...
        Button {
            ...
            Label(approved ? L("Апрувнуто") : L("Апрув"), systemImage: approved ? "checkmark.seal.fill" : "checkmark.seal")
                .foregroundStyle(approved ? Color(nsColor: Theme.gitAdded) : .primary)
        }
        .disabled(busy)
        .help(approved ? L("Отозвать апрув") : L("Одобрить эту версию MR"))
    }
}
            ...
            HStack(spacing: 10) {
                ...
                if let file, let index {
                    if file.raw.deletedFile {
                        Text(L("Файл удалён")).foregroundStyle(Color(nsColor: Theme.gitDeleted))
                    } else if file.isDiffMissing {
                        Text(L("Дифф слишком большой")).foregroundStyle(.secondary)
                    }
                    Text(L("\(index + 1) из \(active.files.count)")).monospacedDigit().foregroundStyle(.secondary)
                    Button {
                        workspace.review.toggleViewed(file)
                    } label: {
                        Label(L("Просмотрен"), systemImage: active.viewed.contains(file.id) ? "checkmark.circle.fill" : "circle")
                    }
                    .help(L("Отметить файл просмотренным"))
                } else if workspace.document != nil {
                    Text(L("рабочая копия")).foregroundStyle(.tertiary)
                }
                Button { workspace.openAdjacentReviewFile(-1) } label: { Image(systemName: "chevron.up") }
                    .help(KeymapStore.shared.help(L("Предыдущий файл MR"), .previousReviewFile))
                Button { workspace.openAdjacentReviewFile(1) } label: { Image(systemName: "chevron.down") }
                    .help(KeymapStore.shared.help(L("Следующий файл MR"), .nextReviewFile))
                ApproveButton(review: workspace.review)
                Button { workspace.closeReview() } label: { Image(systemName: "xmark") }
                    .help(L("Закончить ревью"))
            }
            .buttonStyle(.borderless)
            .font(.system(size: 11))
            .padding(.horizontal, 12)
    ...
    var body: some View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 10) {
                ...
                if let file {
                    CommentComposer(placeholder: threads.isEmpty ? L("Комментарий к строке…") : L("Новый тред к строке…"),
...
struct RemovedLinesView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(lines.count == 1 ? L("Было:") : L("Было (\(lines.count) строк):"))
                .font(.system(size: 11))
                .foregroundStyle(.secondary)
            ScrollView(.horizontal, showsIndicators: false) {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    if lines.count > limit {
                        Text(L("… ещё \(lines.count - limit)")).font(.system(size: 11)).foregroundStyle(.tertiary)
                    }
                }
                .padding(8)
            }
...
struct ThreadCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            if outdated {
                Label(L("К предыдущей версии MR — строка могла сдвинуться"), systemImage: "clock.arrow.circlepath")
                    .font(.system(size: 10))
                    .foregroundStyle(.secondary)
            }
            ...
            HStack(spacing: 12) {
                Button(replying ? L("Отмена") : L("Ответить")) { replying.toggle() }
                if discussion.isResolvable {
                    Button(discussion.isResolved ? L("Открыть снова") : L("Решить")) { toggleResolved() }
                        .disabled(busy)
                }
                Spacer()
                if discussion.isResolved {
                    Label(L("Решён"), systemImage: "checkmark.circle.fill")
            ...
            if replying {
                CommentComposer(placeholder: L("Ответ…"), submitTitle: L("Ответить"), autofocus: true) { body in
...
struct ConnectGitLabView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text(L("Подключение к GitLab")).font(.system(size: 15, weight: .semibold))
            Text(L("Pilot ходит в API GitLab с вашим Personal Access Token. Токен хранится в связке ключей macOS и уходит только на указанный хост. Для комментариев и апрувов нужны права api."))
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            ...
            Grid(alignment: .leading, horizontalSpacing: 10, verticalSpacing: 8) {
                GridRow {
                    Text(L("Хост")).foregroundStyle(.secondary)
            ...
            if let tokenPage, !host.isEmpty {
                Link(L("Создать токен в GitLab ↗"), destination: tokenPage).font(.system(size: 12))
            }
            if let error {
