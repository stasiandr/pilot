...
struct StartView: View {
    ...
    var body: some View {
        ...
        .padding(24)
        // Высота — по содержимому: девять недавних проектов в 400 точек
        // не влезают, и список вылезал за панель.
        .frame(width: projects.isEmpty ? 380 : 760)
        .frame(minHeight: 400)
    ...
    private var header: some View {
        VStack(spacing: 18) {
            ...
            VStack(spacing: 5) {
                ...
                Text(projects.isEmpty ? L("Откройте папку проекта, чтобы начать")
                                      : L("Мгновенный просмотрщик кода"))
                    .font(.system(size: 13))
        ...
        VStack(alignment: .leading, spacing: 1) {
            ...
            ForEach(Array(projects.enumerated()), id: \.element.path) { index, url in
                ProjectRow(url: url, shortcut: index + 1, isSelected: index == selection)
                    ...
                    .onTapGesture { workspace.openProject(url) }
                    .contextMenu {
                        Button(L("Открыть")) { workspace.openProject(url) }
                        Button(L("Показать в Finder")) {
                            NSWorkspace.shared.activateFileViewerSelecting([url])
                        }
                        Divider()
                        Button(L("Убрать из списка")) { workspace.forgetRecent(url) }
                    }
            }
            Spacer(minLength: 0)
        }
    ...
    private var footer: some View {
        VStack(spacing: 10) {
            ...
            .controlSize(.large)
            Text(projects.isEmpty ? "⌘O" : L("↑↓ выбрать · ⏎ открыть · ⌘1–9"))
                .font(.system(size: 11))
                .foregroundStyle(.tertiary)
        }
    }
    ...
    private func installKeyMonitor() {
        ...
        keyMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { event in
            ...
            guard NSApp.modalWindow == nil, !(event.window is NSPanel), workspace.owns(event),
            ...
            if flags == .command, let ch = event.charactersIgnoringModifiers,
               let digit = Int(ch), (1...projects.count).contains(digit) {
                workspace.openProject(projects[digit - 1])
                return nil
            }
            ...
            case 36, 76:                                           // Return
                workspace.openProject(projects[selection]); return nil
            default:
                return event
            }
        }
...
private struct ProjectRow: View {
    ...
    private var parentPath: String {
        // У корня диска родителя нет — `deletingLastPathComponent` дал бы `/..`.
