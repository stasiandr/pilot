...
struct JumpBar: View {
    ...
    private var history: some View {
        HStack(spacing: 2) {
            ...
            .help(KeymapStore.shared.help(L("Назад"), .back))
            ...
            .help(KeymapStore.shared.help(L("Вперёд"), .forward))
        }
    ...
    private func symbolSegment(_ document: LoadedDocument) -> some View {
        segment {
            ...
            if let item = workspace.caret.outlineItem {
                ...
                SegmentLabel(title: document.outline.isEmpty ? L("Нет структуры") : L("Нет выделения"),
                             dimmed: true)
            }
        }
    ...
    private var projectMenu: some View {
        ...
        Divider()
        Button(L("Открыть папку…")) { workspace.promptForFolder() }
    }
    ...
    private func siblingsMenu(parent: String) -> some View {
        ...
        if siblings.count > menuLimit {
            Text(L("… и ещё \(siblings.count - menuLimit)"))
        }
    }
