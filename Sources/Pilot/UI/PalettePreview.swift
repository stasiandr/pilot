...
final class PreviewLoader: ObservableObject {
    ...
    nonisolated static let imageExtensions: Set<String> = [
        "png", "jpg", "jpeg", "gif", "webp", "heic", "tif", "tiff", "bmp", "ico", "icns", "psd", "pdf",
    ]

    /// `document` — открытый в редакторе файл: он уже разобран, диск не нужен.
    func show(_ target: NavTarget?, document: LoadedDocument?) {
    ...
    private static func content(model: SyntaxModel, range: LSPRange?) -> PreviewContent {
        if model.units.isEmpty { return .message("Пустой файл") }
    ...
    private nonisolated static func load(_ url: URL) -> Result<Loaded, Error> {
...
struct PalettePreview: View {
    ...
    private func lineLabel(_ preview: FilePreview) -> String {
        if let line = preview.focusLine { return "стр. \(line + 1) из \(preview.lineCount)" }
