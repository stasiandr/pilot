...
struct TabBar: View {
    ...
    private var barHeight: CGFloat { chrome.isCompact ? 25 : 30 }
    ...
    private func menu(for tab: TextBuffer, at index: Int, path: String) -> some View {
        if tab === workspace.previewTab {
            Button(L("Оставить вкладку открытой")) { workspace.keepTabOpen(tab) }
            Divider()
        }
        Button(L("Закрыть вкладку")) { workspace.closeTab(tab) }
        Button(L("Закрыть другие вкладки")) { workspace.closeOtherTabs(tab) }
            .disabled(workspace.tabs.count < 2)
        Button(L("Закрыть вкладки справа")) { workspace.closeTabsToTheRight(of: tab) }
            .disabled(index == workspace.tabs.count - 1)
        Divider()
        Button(L("Показать в Finder")) { NSWorkspace.shared.activateFileViewerSelecting([tab.url]) }
            .disabled(tab.isReviewVersion || tab.document.decompiled == .jadx)
...
private struct TabItem: View {
    ...
    static func assemblyHint(_ decompiled: LoadedDocument.Decompiled) -> String {
        switch decompiled {
        case .assembly:
            return L("Объявления из сборки — только для чтения")
        case .methodBody(let name):
            return L("\(name): IL из метаданных — только для чтения")
        case .jadx:
            return L("Декомпилировано jadx — только для чтения")
        }
    }
    ...
    var body: some View {
        ...
        HStack(spacing: 5) {
            ...
            if tab.isReviewVersion {
                Image(systemName: "arrow.triangle.pull")
                    ...
                    .help(L("Версия из мерж-реквеста — только для чтения"))
        ...
        .help(isPreview ? L("\(path)\nВременная вкладка — двойной клик оставит её открытой") : path)
        .accessibilityElement(children: .combine)
        .accessibilityAddTraits(isActive ? [.isButton, .isSelected] : .isButton)
        .accessibilityLabel(([name] + (isPreview ? [L("временная")] : []) + (isDirty ? [L("не сохранено")] : []))
            .joined(separator: ", "))
    }

    private var closeHelp: String {
        let close = KeymapStore.shared.help(L("Закрыть вкладку"), .closeTab)
        ...
        ZStack {
            if hovering || (isActive && !isDirty) {
                ...
                Circle()
                    ...
                    .help(L("Есть несохранённые изменения (⌘S)"))
            }
        }
        .frame(width: 16, height: 16)
    }
