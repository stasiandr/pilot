...
struct NavigatorView: View {
    ...
    var body: some View {
        content
            // Свой id, не "pilot.main": с тем же id элемент получал нулевую
            // ширину и уезжал в » на краю тулбара, а простой .toolbar
            // выключал настройку тулбара всего окна.
            .toolbar(id: "pilot.sidebar") {
                ToolbarItem(id: "navigator", placement: .automatic) { tabBar }
            }
            .pilotEdgeBar(.bottom) {
    ...
    private var projectTree: some View {
        if workspace.fileTree == nil {
            placeholder(workspace.root == nil ? L("Папка не открыта") : L("Индексация…"))
        } else {
            ...
            FileTreeView(tree: filtering ? workspace.filteredTree : workspace.fileTree,
                         ...
                         revealRequest: workspace.treeRevealRequest,
                ...
                .overlay {
                    if filtering, workspace.filteredTree?.fileCount == 0 {
                        placeholder(L("Ничего не найдено"))
                    }
                }
        }
    }
    ...
    private var filterField: some View {
        HStack(spacing: 6) {
            ...
            TextField(workspace.navigatorTab == .review ? L("Поиск мерж-реквестов") : L("Фильтр"),
                      text: filterBinding)
                .textFieldStyle(.plain)
                .font(.system(size: 12))
                .help(workspace.navigatorTab == .review
                      ? L("Слова из названия, ветки, меток или имени; !123 — номер, @логин — автор или ревьюер. Слитые и закрытые ищутся в GitLab.")
                      : "")
...
struct OutlineList: View {
    ...
    var body: some View {
        if let document = workspace.document {
            ...
            if items.isEmpty {
                empty(document.outline.isEmpty ? L("В этом файле объявлений не найдено") : L("Ничего не найдено"))
            } else {
            ...
            empty(L("Откройте файл, чтобы увидеть его структуру"))
        }
    }
