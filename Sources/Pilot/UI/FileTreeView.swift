...
struct FileTreeView: NSViewRepresentable {
    ...
    /// Меняется — открытый файл выделяется заново, даже если в дереве уже
    /// ходили стрелками, и фокус клавиатуры переходит в дерево.
    var revealRequest = 0
    ...
    func updateNSView(_ scroll: NSScrollView, context: Context) {
        ...
        coordinator.reveal(request: revealRequest, path: selectedPath, object: selectedObject)
    }
    ...
    final class Coordinator: NSObject, NSOutlineViewDataSource, NSOutlineViewDelegate, NSMenuDelegate {
        ...
        /// Цвета иконок и git ячейка берёт при настройке — сменили схему,
        /// настраиваем видимые строки заново; раскрытое и выделение остаются.
        @objc private func colorSchemeChanged() {
            guard let outline, outline.numberOfRows > 0 else { return }
            outline.reloadData(forRowIndexes: IndexSet(integersIn: 0..<outline.numberOfRows), columnIndexes: [0])
        }
        ...
        private var revealRequest = 0

        func reveal(request: Int, path: String?, object: Int64?) {
            guard request != revealRequest else { return }
            revealRequest = request
            pendingOpen = nil
            shownSelection = path
            shownObject = object
            select(path: path, object: object, scroll: true)
            // Панель могла только что выехать: окна у дерева ещё нет.
        ...
        func menuNeedsUpdate(_ menu: NSMenu) {
            ...
            if let item = outline.item(atRow: outline.clickedRow) as? HierarchyItem {
                let copy = NSMenuItem(title: L("Скопировать путь в иерархии"), action: #selector(copyHierarchyPath(_:)),
                                      keyEquivalent: "")
                copy.target = self
            ...
            let reveal = NSMenuItem(title: L("Показать в Finder"), action: #selector(revealInFinder(_:)), keyEquivalent: "")
            let copyRel = NSMenuItem(title: L("Скопировать путь"), action: #selector(copyPath(_:)), keyEquivalent: "")
            let copyAbs = NSMenuItem(title: L("Скопировать полный путь"), action: #selector(copyAbsolutePath(_:)), keyEquivalent: "")
        ...
        func outlineView(_ outlineView: NSOutlineView, toolTipFor cell: NSCell, rect: NSRectPointer,
            ...
            if let item = item as? HierarchyItem {
                ...
                return item.isPrefab ? L("\(path) — вложенный префаб") : path
            }
