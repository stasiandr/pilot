...
final class FileTree: @unchecked Sendable {
    ...
    /// Только то, что лежит в самом корне: одно чтение папки, без обхода.
    /// Это дерево видно с первого кадра, пока строится настоящее; папки в
    /// нём без содержимого — его приносит полный список файлов.
    ///
    /// Фильтр тот же, что у обхода, — `ignore` и `exclude`: иначе верхний
    /// уровень поменялся бы, когда придёт полное дерево.
    static func shallow(root: URL, ignore: IgnoreMatcher,
                        exclude: ((String, Bool) -> Bool)? = nil) -> FileTree {
        let top = Node(name: "", relPath: "", isDirectory: true, parent: nil)
        var lookup: [String: Node] = ["": top]
        var fileCount = 0
        ...
        for url in entries {
            ...
            if ignore.isIgnored(relPath: name, name: name, isDir: isDirectory) { continue }
            if let exclude, exclude(name, isDirectory) { continue }
            let node = Node(name: name, relPath: name, isDirectory: isDirectory, parent: top)
            top.children.append(node)
            lookup[name] = node
            if !isDirectory { fileCount += 1 }
        }
        sortRecursively(top)
        return FileTree(root: top, fileCount: fileCount, lookup: lookup)
    }
