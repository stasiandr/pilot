...
final class GitService: ObservableObject {
    ...
    func markResolved(_ url: URL) async -> String? {
        ...
        guard let repo = Git.repositoryRoot(for: fileURL.deletingLastPathComponent()) else {
            return L("Файл не в git-репозитории")
        }
        let path = String(fileURL.path.dropFirst(repo.path.count + 1))
        let output = await Task.detached(priority: .userInitiated) {
            Git.run(["add", "--", path], in: repo)
        }.value
        guard let output else { return L("Не удалось запустить git") }
        guard output.status == 0 else { return L("git add завершился с кодом \(output.status)") }
        refresh()
        return nil
    }
