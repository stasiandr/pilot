...
enum Git {
    ...
    /// Результат команды, которая что-то меняет: код, вывод и ошибки.
    struct Result {
        var status: Int32
        var stdout: String
        var stderr: String
        ...
        /// Что показать человеку при отказе: git пишет причину в stderr,
        /// хуки — куда придётся.
        var message: String {
    ...
    /// Запуск для записи: коммит, add, apply, push. В отличие от `run`, берёт
    ...
    static func execute(_ arguments: [String], in directory: URL, input: Data? = nil) -> Result? {
        ...
        // Обе трубы — параллельно: git, упёршийся в полный stderr, не дочитал
        // бы stdout, и наоборот.
        var errorData = Data()
